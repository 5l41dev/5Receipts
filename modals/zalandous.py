...
def is_zalando_link(link):
    zalando_pattern = re.compile(r'^https?://(www\.)?(zalando\.com|en\.zalando\.de)(/.+)?')


    return bool(zalando_pattern.match(link))


class zalandomodal(ui.Modal, title="5Receipts"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if user_details:
            ...
            if not is_zalando_link(Link):
                embed = discord.Embed(title="Error - Invalid Zalando link", description="Please provide a valid Zalando link.", color=PURPLE)
