...
class applemodal(ui.Modal, title="5Receipts"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            if not (re.match(r'^\d+(\.\d{1,2})?$', self.Shipping.value)):
                embed = discord.Embed(title="❌ Invalid Shipping Format", description="Please use a valid format (e.g., 12.94) for Apple Shipping Fee.", color=PURPLE)
