...
def is_crtz_link(link):
    crtz_pattern = re.compile(r'^https?://(?:www\.)?crtz\.xyz/.+$')

    return bool(crtz_pattern.match(link))


class crtzmodal(ui.Modal, title="5Receipts"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        global Linkff , Priceff, currencyff, name
        ...
        if user_details:
            ...
            if not is_crtz_link(Linkff):
                embed = discord.Embed(title="Error - Invalid Corteiz link", description="Please provide a valid Corteiz link.", color=PURPLE)
            ...
            try:
                ...
                with open("receipt/crtz.html", "r", encoding="utf-8") as file:
                ...
                response = requests.get(
                    url=url,
                    proxies={
                        "http": f"http://{ZYTE_KEY}:@api.zyte.com:8011/",
                        "https": f"http://{ZYTE_KEY}:@api.zyte.com:8011/",
                    },
                    verify='zyte-ca.crt' 
                )
                ...
                with open("receipt/updatedrecipies/updatedcrtz.html", "w", encoding="utf-8") as file:
                    file.write(html_content)


                sender_email = "CRTZRTW <noreply@corteiz.com>"
                subject = "Order #230750 confirmed"
