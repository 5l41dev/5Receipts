...
class tnfmodal(ui.Modal, title="5Receipts"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if user_details:
            ...
            if not is_tnf_link(Linkff):
                embed = discord.Embed(title="Error - Invalid  link", description="Please provide a valid The North Face link.", color=PURPLE)
                await interaction.response.send_message(embed=embed, ephemeral=True)
                return

            
            embed = discord.Embed(title="🧩 One Last Step!", description="Complete the next steps to receive the receip.", color=PURPLE)
