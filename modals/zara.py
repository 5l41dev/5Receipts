...
class zaramodal(ui.Modal, title="5Receipts"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if not is_zara_link(Link):
            embed = discord.Embed(title="Error - Invalid Zara link", description="Please provide a valid Zara link.", color=PURPLE)
            await interaction.response.send_message(embed=embed, ephemeral=True)
            return

        
        embed = discord.Embed(title="📋 Almost Done!", description="One more quick form and your receipt is ready to send. 🧾", color=PURPLE)
