...
class stockxmodal(ui.Modal, title="5Receipts"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if not is_stockx_link(link):
            embed = discord.Embed(title="Error - Invalid StockX link", description="Please provide a valid StockX link.", color=PURPLE)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            if not (re.match(r'^\d+(\.\d{1,2})?$', pfee1) and re.match(r'^\d+(\.\d{1,2})?$', shipping1)):
                embed = discord.Embed(title="Error StockX - Invalid fee/shipping format", description="Please use a valid format (e.g., 12.94) for StockX Fee and Shipping Fees.", color=PURPLE)
